...
public class ScriptConsole extends GameSubWindow {
	...
	private void executeScript(String text) {
		...
		try {
			...
			if (exception == null) {
				if (returnValue != null) appendText("green", returnValue.toString());
				else appendText("green", "Success.");
			} else {
	...
	private void appendStringFormatted(String text) {
		for (int i = 0; i < text.length(); i++) {
			...
			switch (ch) {
			case '<': textAreaContent.append("&lt;"); break;
			case '>': textAreaContent.append("&gt;"); break;
			case '&': textAreaContent.append("&amp;"); break;
			case '"': textAreaContent.append("&quot;"); break;
			default:
		...
		@Override public boolean handleEvent(Event evt) {
			...
			case KEY_PRESSED:
				...
				case Event.KEY_UP:
					...
					if (previousCommandIndex >= previousCommands.size()) previousCommandIndex = previousCommands.size() - 1;
