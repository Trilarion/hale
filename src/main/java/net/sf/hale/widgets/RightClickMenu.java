    ...
    private class MenuContent extends Widget {
        ...
        protected void layout() {
            ...
            for (RightClickMenuLevel level : levels) {
                ...
                if (parentLevel == null) level.setPosition(getInnerX(), getInnerY());
                else {
                    Widget button = level.getParentButton();
                    if (button == null) level.setPosition(parentLevel.getRight(), parentLevel.getY());
                    else level.setPosition(parentLevel.getRight(), button.getY() - level.getButtonsOffsetY());
                }
