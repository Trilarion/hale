...
public class Transition implements Saveable {
	...
	public EndPoint getEndPointForCreaturesInCurrentArea() {
		if (Game.curCampaign.curArea.getID().equals(to.getAreaID())) {
			return from;
		} else if (Game.curCampaign.curArea.getID().equals(from.getAreaID())) {
			return to;
		} else {
			return null;
		}
	...
	public EndPoint getEndPointInArea(Area area) {
		if (area.getID().equals(to.getAreaID())) {
			return to;
		} else if (area.getID().equals(from.getAreaID())) {
			return from;
		} else {
			return null;
		}
	...
	public boolean isTwoWay() { return isTwoWay; }
	...
	private static class WorldMapEndPoint implements EndPoint {
		...
		@Override public int getY() { return -1; }

		@Override public Iterator<PointImmutable> getPartyPositionsIterator() {
